...
public class RangesState : BaseState
{
    ...
    IEnumerator BasicAttack()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            // L�gica para lanzar un proyectil hacia el jugador
            owner.FireProjectile(owner.PlayerRef.transform.position);
            yield return new WaitForSeconds(timeBetweenAttacks);
        }

        // Transici�n autom�tica a AreaAttack despu�s de 2 ataques b�sicos
        GoToAreaAttackSubstate();
    }
    ...
    IEnumerator AreaAttack()
    {
        ...
        {
            // L�gica para lanzar un proyectil en abanico hacia el jugador
            owner.FireFanAttack(owner.PlayerRef.transform.position);
            yield return new WaitForSeconds(timeBetweenAttacks);
        }

        // Transici�n a Ultimate si se han realizado todos los ataques
    ...
    {
        BossEnemy bossEnemy = (BossEnemy)owner;
        Vector2[] spawnPoints = GetRandomSpawnPoints(10); // Generar 10 puntos de aparici�n aleatorios

        foreach (Vector2 point in spawnPoints)
        {
            // L�gica para lanzar un proyectil grande en un punto espec�fico
    ...
    Vector2[] GetRandomSpawnPoints(int numPoints)
    {
        ...
        {
            float x = Random.Range(-10f, 10f); // Ajustar el rango seg�n sea necesario
    ...
    {
        _substateHistory.Add(_rangedSubstate);
        _rangedSubstate = RangedSubstate.BasicAttack; // Transici�n de vuelta a BasicAttack por dise�o
        SubstateEntered = false;
    }
}
